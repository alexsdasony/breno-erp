  ...
  try {
    ...
    if (!columns.includes('status')) {
      ...
      await new Promise((resolve, reject) => {
        db.run('ALTER TABLE users ADD COLUMN status TEXT DEFAULT "ativo"', (err) => {
          if (err) reject(err);
          else resolve();
        });
      });
    ...
    if (!columns.includes('segment_id')) {
      console.log('➕ Adicionando coluna segment_id...');
      await new Promise((resolve, reject) => {
        db.run('ALTER TABLE users ADD COLUMN segment_id INTEGER', (err) => {
          if (err) reject(err);
          else resolve();
        });
      });
    ...
    if (existingAdmin) {
      ...
      console.log(`   Status: ${existingAdmin.status || 'N/A'}`);
